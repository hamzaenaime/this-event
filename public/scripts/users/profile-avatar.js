...
(() => {
  ...
  function displayValidationErrors(jsonObject, container) {
    ...
    $(jsonObject.validationErrors).each(function(index, item) {
      container.find("ul").append($(document.createElement("li")).text(item));
    });
  }
    ...
    $profileForm.find("#form-file").each(function() {
      ...
      if (inputName === "file") {
        ...
        if (!file) {
          ...
          input.next("span").text("Choisir une fichier pour l'uploader");
        }
        ...
        if (file.name.match(/\.(jpg|jpeg|png)$/i)) {
          ...
          input.addClass("input-error");
          input.next("span").text("type de fichier autoriser: jpg, jpeg, png.");
        }
