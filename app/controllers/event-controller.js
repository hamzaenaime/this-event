...
module.exports = function(data) {
  return {
    createEvent(req, res) {
      ...
      return data
        .createEvent(req.body, req.user)
        .then(event => {
          res.status(200).send({ redirectRoute: "/events", eventId: event.id });
        })
        .catch(err => {
          res
            .status(400)
            .send(
              JSON.stringify({ validationErrors: helpers.errorHelper(err) })
            );
        });
    },
    commentEvent(req, res) {
      return data
        .commentEvent(req.params.id, req.body.commentText, req.user)
        .then(commentData => {
    ...
    subscribeOrUnsubscribeForEvent(req, res) {
      ...
      if (req.isAuthenticated()) {
        if (!userHasAlreadySubscribed(req.user.subscribedEvents, eventId)) {
          ...
          return data
            .subscribeForEvent(eventId, req.user.id)
            .then(() => {
              res.status(200).send({ userHasSubscribed });
            })
            .catch(err => {
              res
                .status(400)
          ...
          return data
            .unsubscribeForEvent(eventId, req.user.id)
            .then(() => {
              res.status(200).send({ userHasSubscribed });
            })
            .catch(err => {
    ...
    uploadImage(req, res) {
      ...
      return new Promise((resolve, reject) => {
            ...
            if (!part.filename || part.filename.match(/\.(jpg|jpeg|png)$/i)) {
              form.on("end", function(fields, files) {
                if (this.openedFiles[0].size > form.maxFieldsSize) {
                  ...
                  res.status(200).send({ redirectRoute: "/events" });
                }

                let eventFolder = eventId,
                  pathToUploadFolder = path.join(
                    __dirname,
                ...
                data
                  ...
                  .then(newFileName => {
                    let uploadedFileName = uploader.uploadFile(
                      this.openedFiles[0],
                      pathToUploadFolder,
                      newFileName
                    );
        ...
        .then(fileName => {
          ...
          let eventImgUrl =
              "/static/uploads/events/" + eventId + "/" + fileName,
    ...
    getSpecificEvents(req, res) {
      data
        .getSpecificEvents(COUNT_OF_EVENTS)
        .then(events => {
          res.send(
