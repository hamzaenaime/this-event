  ...
  return {
    ...
    getProfileAvatar(req, res) {
      return Promise.resolve().then(() => {
        if (!req.isAuthenticated()) {
          ...
          if (req.user.role === "admin") {
            res.render("user/profile-avatar", {
              user: req.user,
              isAdmin: true
            });
          } else {
    ...
    uploadProfileAvatar(req, res) {
      return new Promise((resolve, reject) => {
            ...
            if (!part.filename || part.filename.match(/\.(jpg|jpeg|png)$/i)) {
              form.on("end", function(fields, files) {
                if (this.openedFiles[0].size > form.maxFieldsSize) {
                  ...
                  res.status(200).send({ redirectRoute: "/profile" });
                }
        ...
        .then(fileName => {
          ...
          let avatarUrl =
            "/static/uploads/users/" + req.user.id + "/" + fileName;
    ...
    getAllEventsForApproval(req, res) {
      if (!req.isAuthenticated() || req.user.role !== "admin") {
        ...
        return data.getAllAwaitingEvents().then(events => {
          res.render("user/approve-events", {
            user: req.user,
            isAdmin: true,
            events: events
          });
    ...
    getAllUserEvents(req, res) {
      return Promise.resolve().then(() => {
        if (!req.isAuthenticated()) {
          ...
          return data.getUserByName(req.user.username).then(user => {
